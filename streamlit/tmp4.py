...
def load_data():
    ...
    df['track_genre'] = df['track_genre'].apply(lambda x: [i.strip() for i in str(x).split(",")])
    exploded_track_df = df.explode('track_genre')

    # Normalize loudness and tempo using their specific ranges
    exploded_track_df['loudness'] = (exploded_track_df['loudness'] + 60) / 60
    exploded_track_df['tempo'] = (exploded_track_df['tempo'] - 60) / 180
...
all_genres = sorted(exploded_track_df['track_genre'].unique())
...
def n_neighbors_uri_audio(genre, test_feat):
    test_feat[-4] = (test_feat[-4] + 60) / 30
    test_feat[-1] = (test_feat[-1] - 60) / 90
    ...
    track_ids = genre_data.iloc[n_neighbors]["track_id"].tolist()
...
playlist_id = st.text_input("Enter your Spotify Playlist ID:")
access_token = st.text_input("Enter your Spotify Access Token:", type="password")  # Token input

if st.button("Generate Playlist CSV"):
    # if playlist_id and access_token:
    #     output_file = "playlist_data.csv"
    #     generate_playlist_csv(playlist_id, access_token, output_file)
    #     st.success(f"CSV generated successfully! Saved as {output_file}.")
    # else:
    #     st.error("Please provide both Playlist ID and Access Token.")
    st.error("Sadly, Spotify has disabled track audio features API since Nov 27. (https://developer.spotify.com/blog/2024-11-27-changes-to-the-web-api)gi")
...
if st.session_state.get('step', 1) >= 3:
    ...
    with col1:
        ...
        if search_button:
            ...
            st.session_state['recommendations'] = (track_ids, audios)

    with col2:
        select_image_button = st.button('Select Another Image')
        if select_image_button:
            # Set reset_uploader to True and restart the app
            st.session_state['reset_uploader'] = True
            st.experimental_rerun()

    # Display recommendations
    track_ids, audios = st.session_state['recommendations']
    if track_ids:
        tracks_per_page = 6
        tracks = []
