...
def load_data():
    ...
    df['genres'] = df['genres'].apply(lambda x: [i.strip() for i in str(x).split(",")])  # Assuming genres are comma-separated
    exploded_track_df = df.explode('genres')  # Explode the genre column
    return exploded_track_df

exploded_track_df = load_data()

# Get a list of all unique genres
all_genres = sorted(exploded_track_df['genres'].unique())  # Updated to use 'genres'
print(all_genres)
